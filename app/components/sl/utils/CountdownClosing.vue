<template>
  ...
  <section class="tag is-large is-white has-text-black is-rounded" v-if="(new Date(date)) > (new Date())">
    <p>{{what}} cierra en
      <i class="fas fa-clock"></i> {{days}} días, {{hours}}:{{minutes}}:{{seconds}}</p>
  </section>
  <section class="" v-else>
     <p class="title is-3 shadow-text">¡{{what}} ha cerrado! ¡Gracias por participar!</p>
  </section>
</template>
